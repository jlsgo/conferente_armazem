...
## Impressao unificada do fechamento diario - prioridade alta

Adicionar um botao unico, **Imprimir fechamento completo do dia**, para buscar os
dados de todas as abas do armazem selecionado e gerar um unico relatorio para
impressao ou salvamento em PDF. As consultas dos tres fluxos devem ser feitas em
paralelo para reduzir o tempo de espera.

O relatorio deve conter secoes independentes:

### Saidas de Armazem

- registros de scooters, triciclos e patinetes;
- quantidade de pedidos;
- total de unidades;
- area de assinatura da conferente responsavel.

### Pecas para Montagem

- total de pecas;
- entradas e saidas separadas;
- quantidade por condicao: boa, defeito e sucata;
- area de assinatura do responsavel.
...
Fluxos sem registros devem ser ocultados automaticamente para economizar papel.
Os totais nao devem ser somados em um total geral, porque veiculos e pecas sao
unidades operacionais diferentes. Cada secao deve exibir seu proprio subtotal,
por exemplo:

```text
Saidas de veiculos: 68 unidades
Pecas para montagem: 12 unidades
Pecas SAC: 71 unidades
Vendas SAC: R$ 2.928,69
```

O fechamento deve oferecer dois formatos:

- **Compacto:** voltado para assinatura e economia de papel, com apenas as
  colunas essenciais, cabecalho repetido e pagina em A4 paisagem.
- **Detalhado:** voltado para auditoria, preservando observacoes, descricoes
  completas, rastreio e hash de auditoria abreviado.

Regras de impressao do formato compacto:

- A4 horizontal, margens aproximadas de 6 a 8 mm e escala de 100%;
- fonte aproximada de 8 a 9 px e espacamento reduzido nas celulas;
- repeticao do cabecalho da tabela em cada pagina;
- evitar quebra de um registro entre paginas;
- quebra de pagina entre fluxos quando necessario;
- assinatura ao final de cada secao ou na ultima pagina da secao;
- ocultar controles da interface, cabecalho e rodape do navegador;
- manter os dados completos disponiveis no formato detalhado.

Quarenta registros por folha e uma meta realista. Cinquenta podem caber quando
as descricoes forem curtas, mas a legibilidade deve ter prioridade sobre uma
quantidade fixa de linhas. Descricoes longas podem ocupar mais de uma linha ou
levar a secao para uma pagina adicional.

O teste de aceite deve usar dias com aproximadamente 40 e 50 registros, com
descricoes curtas e longas, verificando que os subtotais, o total liquido, as
assinaturas e o hash continuam corretos. A impressao deve ser testada em uma
impressora real e na opcao de salvar como PDF.
...
- o fechamento completo do dia puder ser impresso por um unico comando, com
  subtotais separados para cada fluxo e sem imprimir abas vazias;
...
8. Impressao unificada compacta e detalhada do fechamento diario.
